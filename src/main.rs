...
fn main() {
    ...
    // 6. DeflateDecoder (0.62s native)
    let now = SystemTime::now();
    let mut output_6 = Vec::with_capacity(10_000_000);
    let mut deflate_decoder = DeflateDecoder::new(buffer.as_slice());
    deflate_decoder.read_to_end(&mut output_6).expect("6");
    println!("Elapsed (with_capacity): {:?}", now.elapsed());

    // 7. libflate (2.2s native, 4.8s wasm)
    let now = SystemTime::now();
    let mut output_7 = Vec::new();
    let mut libflate_decoder = deflate::Decoder::new(buffer.as_slice());
    libflate_decoder.read_to_end(&mut output_7).expect("7");
